...
export class IntroSequence {
    ...
    private drawSpace(time: number): void {
        ...
        this.renderer.drawMesh(
            this.sphere,
            fromTRS([-1.28, -0.05, -14.15], [0.3, 0, 0.2], [1.5, 0.22, 0.8]),
            [0.09, 0.55, 0.28, 1],
            0,
            0.55,
        );
        this.renderer.drawMesh(
            this.sphere,
            fromTRS([1.08, -1.07, -14.08], [-0.2, 0.5, -0.2], [1.9, 0.24, 0.62]),
            [0.1, 0.48, 0.25, 1],
            0,
            0.55,
        );
        this.renderer.drawMesh(
            this.sphere,
            fromTRS([0, -0.55, -16.5], [0, 0, 0], [5.76, 5.76, 5.76]),
            [0.25, 0.85, 1, 0.18],
            0.55,
            0.5,
        );
    ...
    private drawAtmosphere(time: number): void {
        ...
        this.renderer.drawMesh(
            this.plane,
            fromTRS([0, -1.6, -13], [0, 0, 0], [44, 1, 44]),
            [0.67, 0.2, 0.07, 0.28 * glow],
            1.1,
            1.7,
        );
        for (let i = 0; i < 11; i += 1) {
            ...
            this.renderer.drawMesh(
                this.box,
                fromTRS([x, y - fall * 7.5, z], [0.8, 0.2, 0.75], [0.08, 0.08, 4.5]),
                [1, 0.36, 0.12, 0.56],
                1.2,
                0.6,
            );
